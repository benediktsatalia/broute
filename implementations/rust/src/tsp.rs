...
impl TSPData {
    pub fn d(&self, i: usize, j: usize) -> i32 {
        self.d[i * self.n + j]
    }

    pub fn aux(&self, i: usize, j: usize) -> f64 {
        self.aux[i * self.n + j]
    }
    ...
    pub fn aux2(&self, i: usize, j: usize) -> f64 {
        self.aux2[i * self.n + j]
    }
...
impl TSPSolution {
    pub fn two_opt(&mut self, data: &TSPData) -> i32 {
        let mut t = 0;
        while self.first_2e_improvement(&data) {
            t += 1;
        }
        return t;
    }
    ...
    fn first_2e_improvement(&mut self, data: &TSPData) -> bool {
        for (p1, n1) in self.nodes.windows(2).enumerate() {
            ...
            for (p2, n2) in self.nodes.windows(2).enumerate().skip(p1 + 2) {
                let (s2, t2) = (n2[0], n2[1]);
                if data.d(s1, t1) + data.d(s2, t2) > data.d(s1, s2) + data.d(t1, t2) {
    ...
    pub fn or_opt(&mut self, data: &TSPData) -> i32 {
        let mut t = 0;
        while self.first_or_improvement(&data) {
            t += 1;
        }
        return t;
    }
    ...
    fn or_delta(
        ...
        return data.d(ipred, esucc) + data.d(pnode, inode) + data.d(enode, psucc)
            - data.d(pnode, psucc)
            - data.d(ipred, inode)
            - data.d(enode, esucc);
    }
    ...
    fn first_or_improvement(&mut self, data: &TSPData) -> bool {
        for (iminus, n1) in self.nodes.windows(2).enumerate() {
            ...
            for l in 1..(1 + min(3, self.nodes.len() - iminus - 2)) {
                let (enode, esucc) = (self.nodes[iminus + l], self.nodes[iminus + l + 1]);
                for (p, n2) in self.nodes.windows(2).enumerate().take(iminus) {
                    let (pnode, psucc) = (n2[0], n2[1]);
                    let delta = self.or_delta(data, ipred, inode, enode, esucc, pnode, psucc);
                    if delta < 0 {
